...
try {
  ...
  try {
    ...
    // A consuming app may define its own #app alias and source condition.
    // The library must resolve its internals in its own package scope.
    writeFileSync(path.join(consumer, 'package.json'), JSON.stringify({ type: 'module', imports: {
      '#app/*': './unrelated/*.js',
    } }));
    const sample = readFileSync(path.join(root, 'test/fixtures/package-consumer.ts'), 'utf8');
    ...
    execFileSync(process.execPath, ['--conditions=source', path.join(consumer, 'out/index.js')], { stdio: 'pipe' });
